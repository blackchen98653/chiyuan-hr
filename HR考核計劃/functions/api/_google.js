// 共用模組：用 Google 服務帳戶存取 Google Sheets API（純 Cloudflare）
...
async function getToken(env) {
  ...
  const unsigned = b64urlStr(JSON.stringify({ alg: "RS256", typ: "JWT" })) + "." +
    b64urlStr(JSON.stringify({ iss: env.GOOGLE_CLIENT_EMAIL, scope: "https://www.googleapis.com/auth/spreadsheets", aud: "https://oauth2.googleapis.com/token", exp: now + 3600, iat: now }));
...
export async function readRange(env, range) {
  const t = await getToken(env), id = sheetId(env);
  const r = await fetch(`https://sheets.googleapis.com/v4/spreadsheets/${id}/values/${encodeURIComponent(range)}`, { headers: { Authorization: "Bearer " + t } });
  const j = await r.json(); return j.values || [];
}
export async function appendRows(env, range, rows) {
  const t = await getToken(env), id = sheetId(env);
  const r = await fetch(`https://sheets.googleapis.com/v4/spreadsheets/${id}/values/${encodeURIComponent(range)}:append?valueInputOption=RAW&insertDataOption=INSERT_ROWS`, { method: "POST", headers: { Authorization: "Bearer " + t, "Content-Type": "application/json" }, body: JSON.stringify({ values: rows }) });
  return r.json();
}
